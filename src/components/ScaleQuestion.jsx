...
export default function ScaleQuestion({ data, value, onChange, onNext }) {
  ...
  return (
    <div className="w-full max-w-xl mx-auto flex flex-col items-center">
      {/* Visual Feedback */}
      <motion.div
        className="text-center mb-10 h-16 flex flex-col items-center justify-end"
      ...
      >
        <motion.span 
          className="text-6xl block mb-2 filter grayscale brightness-150 contrast-125"
          initial={{ scale: 0.8, opacity: 0 }}
          animate={{ scale: 1, opacity: 1 }}
          transition={{ type: "spring", stiffness: 300, damping: 20 }}
        >
          {data.emojis ? data.emojis[localVal - 1] : localVal}
        </motion.span>
      </motion.div>
      ...
      {/* Labels */}
      <motion.div
        className="flex justify-between w-full px-2 mb-10 text-xs sm:text-sm text-[var(--color-text-muted)] tracking-wider uppercase font-medium"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        transition={{ delay: 0.3 }}
      >
        <span>{data.labels?.min || data.min}</span>
        <span>{data.labels?.max || data.max}</span>
      </motion.div>
